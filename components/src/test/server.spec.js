...
//       .request(server)
//       .post('/register')
//       .send({username: 'Example', userPassword: 'password'})
//       .end((err, res) => {
//         expect(res).to.have.status(200);
...
it('Negative : /login. Checking invalid name', done => {
  chai
    .request(server)
    .post('/login')
    .send({usrname: 'fake', password: 'pass'})
    .end((err, res) => {

      assert.strictEqual(res.body.message, 'Username and password are both required for login');
